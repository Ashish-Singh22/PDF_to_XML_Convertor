...
const History = ()=>{
  ...
  const fetchHistoryDetails = async () => {
    try {
      const response = await fetch(SummaryApi.historyDetails.url, {
        method: SummaryApi.historyDetails.method,
        credentials: "include",
        headers: {
      ...
      const pdfLink = dataResponse?.data?.pdfLink;
      setPdfUrl(pdfLink);
      setConvertTo(dataResponse?.data?.type);
      setCode(dataResponse?.data?.code);
      setShowPdfPreview(true);
  
      if (pdfLink) {
        // Fetch the PDF and convert it to a File object
        const pdfResponse = await fetch(pdfLink);
        const pdfBlob = await pdfResponse.blob();
        const pdfFile = new File([pdfBlob], dataResponse?.data?.fileName, { type: "application/pdf" });
        setFile(pdfFile);
      }
    } catch (error) {
